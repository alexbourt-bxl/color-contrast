export function buildSwatchMarkdown(
  params:
  {
...
): string
{
  ...
  const badgesBlockHeight = footerBadges ? calculateBadgesBlockHeight(
  {
    padding,
  }) : 0;
...
`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  ${footerBadges ? buildFooterBadgesSvg(
  {
    width,
    y: size + padding,
    padding,
  ...
  ${footerLines.length > 0 ? buildFooterSvg(
  {
    lines: footerLines,
    x: width / 2,
...
function calculateBadgesBlockHeight(
  params:
  {
    padding: number;
  }
